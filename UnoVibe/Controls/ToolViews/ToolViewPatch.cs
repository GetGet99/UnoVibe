    ...
    <StackPanel Spacing=4>
        <Button Background=`Hovering ? theme.SystemNeutralBackground : theme.SubtleFill` CornerRadius=4 Padding=`new Thickness(8, 4, 8, 4)` BorderThickness=0 HorizontalContentAlignment=Left HorizontalAlignment=Stretch Click+=`(s, e) => Expanded = !Expanded` PointerEntered+=`(s, e) => Hovering = true` PointerExited+=`(s, e) => Hovering = false`>
            <StackPanel Orientation=Horizontal Spacing=8>
                ...
                <TextBlock Text=`ToolViewShared.Patch(Part)` FontSize=12 FontFamily="Consolas" Foreground=`theme.PrimaryText` TextWrapping=Wrap IsTextSelectionEnabled=true VerticalAlignment=Center />
            </StackPanel>
        </Button>
        if (`Expanded`)
        {
            foreach (var f in `ToolViewShared.ParsePatchFiles(Part)`)
                <Border Background=`theme.SolidBackground` CornerRadius=4 Padding=`new Thickness(8, 6, 8, 6)` Margin=`new Thickness(0, 2, 0, 2)`>
                    <StackPanel Spacing=4>
                        <TextBlock Text=`ToolViewShared.PatchFileLine(f)` FontSize=11 FontFamily="Consolas" FontWeight=`FontWeights.SemiBold` Foreground=`theme.PrimaryText` TextWrapping=Wrap IsTextSelectionEnabled=true />
                        if (`f.Patch.Length > 0`)
                            <TextBlock Text=`ToolViewShared.Truncate(f.Patch, 6000)` FontSize=12 FontFamily="Consolas" Foreground=`theme.SecondaryText` TextWrapping=Wrap IsTextSelectionEnabled=true />
                    </StackPanel>
                </Border>
