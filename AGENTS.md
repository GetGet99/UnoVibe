...
- **Unhandled events**: `ChatStore.Apply` has `// TODO:` placeholder `case`s (with `break;`) for every other event the server's `/event` stream emits (`message.removed`, `session.deleted/error/diff/idle/compacted`, `file.edited`, `file.watcher.updated`, `vcs.branch.updated`, `todo.updated`, `lsp.updated`, `command.executed`, `mcp.browser.open.failed`, `server.connected/heartbeat/instance.disposed`, `tui.toast.show`). `session.created`/`session.updated`, `session.status`, `question.replied`/`question.rejected` (pending-attention counters), and `mcp.tools.changed` (→ `RefreshMcpStatusAsync`) are handled. The `session.next.*` streaming events exist in the schema but are not published by the current CLI server. Implement a case and remove its TODO marker when adopting it.
...
- **Chat input suggestion box** (`SuggestBox` in `UnoVibe/Controls/SuggestBox.cs`): a self-contained QuickMarkup component (multiline TextBox + attached suggestion `Flyout`) that offers `@` and `/` completions. `Prefixes` (default `"/@"`) and `Providers` (suggestion sources) are public members; it raises `SubmitRequested(sender, text)` on bare Enter (flyout closed) and offers `Clear()`; every other property/event (`Text`, `PlaceholderText`, `MaxHeight`, `PreviewKeyDown`, ...) forwards to the inner TextBox via `MarkupNode`. The flyout uses `ShowMode=Transient` so it never steals focus on open, plus a `GotFocus` handler on the flyout content that bounces focus back to the input (mirrors RichSuggestBox's `SuggestionList_GotFocus`) — the editor keeps focus for the whole suggestion session. Parsing + dispatch live in `SuggestionBoxController` (`UnoVibe/Controls/`) — every prefix (both `/` and `@`) triggers at start-of-token (start of input or preceded by whitespace), so `foo /skill-name` and `foo @file` work, while `foo/bar`/email `foo@bar` do not; providers route per trigger char. Items flagged `InputStartOnly` on the `SuggestionItem` (built-in whole-input commands like `/new`) are filtered out unless the trigger is at position 0, so `/new` only appears when the input starts with `/`, while skills (`/quickmarkup`) and mentions work anywhere. Row model in `Controls/SuggestionItem.cs`, mock providers in `Services/SuggestionProviders.cs` (`namespace UnoVibe.Controls`). ChatPage consumes it as `<SuggestBox Text<=>`Input` SubmitRequested+=`OnSubmitRequested` />` and supplies the providers. Phase 1 uses mock providers; **Phase 2 (real server data) is specified in `AUTOCOMPLETE_PLAN.md`** — server surface is `/api/command`, `/api/skill`, `/api/fs/find` (LocationQuery `location[directory]` deep-object; responses `Command.Info[]`/`Skill.Info[]`/`{location, data:[{path,type}]}`). The server expands `/name args` into the command template on send (`session/prompt.ts`), so the client just inserts `/name ` and sends plain text.
