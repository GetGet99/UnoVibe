...
> **No-rebuild sidebar model:** sidebar state lives on persistent instances — `SessionHead`
> items and `DirectoryGroup` groups are reused (never recreated). `SessionsStateProvider`
...
> page's subagent strip. Per-session sidebar flags (`SessionHead` fields, keyed by session id)
> stay the authoritative store for busy/outcome/pending-attention because SSE can fire for sessions
...
`SessionsStateProvider` keeps a `Dictionary<string, DirectoryGroup> _groupsByDirectory` index; `DirectoryGroup`
...
`SessionSidebar.RunFolderAction` delegates to `Helpers/FolderLauncherHelper.cs`
...
`Toasts.ShowError`. `Symbol.Code` is defined as `(Symbol)0xe943` in `SymbolExtemsion.cs`.

**Open Folder button:**
The sidebar's **Open Folder** button (a folder picker that starts a new unsaved session in the picked
folder via `SessionsStateProvider.NewSessionAsync`) is a small icon button (`Symbol.Folder`, tooltip "Open Folder")
...
Folders opened with it — or with a group's "+" button — are tracked in `SessionsStateProvider._openedFolders`
...
`SessionsStateProvider.RefreshSessionsAsync` also fetches `GET /session?directory=<path>` for every opened folder and merges
...
Opening a folder also starts a directory-scoped `/event` stream (`EventsProvider.StartFolderEventStream`) so a
...
(`OpencodeConnection.ServerDirectory`), **URL** and **password**, each as a selectable `TextBlock`
...
`Visibility`).
The values come from `OpencodeConnection.ConnectionUrl`/`ConnectionPassword`, reactive fields set in
`OpencodeConnection.Configure` (password resolved with the `OPENCODE_SERVER_PASSWORD` env-var fallback so
