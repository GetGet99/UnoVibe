...
Row model in `Controls/SuggestionItem.cs`, providers in `Commands/SuggestionProviders.cs`
(`namespace UnoVibe.Controls`).

## App built-in commands

`BuiltInCommands` (in `Commands/SuggestionProviders.cs`) is the app-level catalog — TUI parity with
...
- `/fork` → `ChatboxState.ForkFullSessionAsync()` — same full-session fork as the header's ⇆ button
- `/interrupt` → `ChatboxState.InterruptAsync()` when busy; warning toast otherwise (contextually
  hidden in the flyout while idle)
- `/mcps` → `SessionsStateProvider.RequestMcpSection()` → `SessionSidebar.RevealMcpSectionAsync`: on compact
...
- `/new` → `SessionsStateProvider.NewSessionAsync(SessionsStateProvider.ActiveDirectory())` — identical to the "+" button on the
  current session's directory group (lazy draft; created on first send)
- `/redo` → `ChatPage.RedoLastAsync()` → `ChatboxState.RedoLastMessageAsync()` + scroll to end
...
- `/undo` → `ChatPage.UndoLastAsync()` → `ChatboxState.UndoLastMessageAsync()` + restore the undone
...
on the running dev server (opencode **1.18.0**), the legacy `/command?directory=`/
...
The current dev server is 1.18.0 — older than the opencode-src checkout (1.18.11) whose
`/api/command` handler folds skills in via `Command.state`, so the `source == "skill"` mapping stays
as a defensive branch. (Note: this fact is no longer true as user now checks out on 1.18.0)
...
the model unexpanded. So `ChatboxState.SendPromptNowAsync` detects the command client-side
(`ParseSlashCommand`, mirroring the TUI's `prompt/index.tsx`: first line, first space-delimited token,
leading `/` stripped, remaining tokens join the arguments) and, when `SessionsStateProvider.IsKnownCommandAsync`
...
immediately. The command-name cache in `SessionsStateProvider` is invalidated on directory change or a 5-minute
...
`SessionsStateProvider.IsKnownCommandAsync` caches the command list split into real-command names vs
...
plain prompt. Arguments: `ChatboxState.ParseSlashCommand` mirrors the TUI — first line, first
